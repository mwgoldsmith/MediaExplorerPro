...
public:
  ...
  enum Flag {
    ...
    Clamped = 1 << 2, 
    ///< Forces the value to be a multiple of 8
    EightMultiple = 1 << 3, 
    ///< Forces the value not to be empty (likely to be used only with Strings)
    NotEmpty = 1 << 4, 
    ///< Defines a variable that is not meant to be saved
    Runtime = 1 << 5, 
    ///< Specifies that a setting should only contain folders.
    Folders = 1 << 6
  };
  ...
  */
  SettingValue(const QString& key, Type type, const QVariant& defaultValue, const char* name, const char* desc, Flags flags = Nothing);
...
protected:
  /**
  * @brief the QVariant containing the value of the settings
  */
  QVariant        m_val;
...
signals:
  /**
  * @brief This signal is emmited while the m_val member have been modified
  */
