...
#ifndef LOGGING_LOGAVCODEC_H
#define LOGGING_LOGAVCODEC_H
...
static struct {
  ...
  { AV_CODEC_ID_VOXWARE,           "AV_CODEC_ID_VOXWARE"           },
  ...
  { static_cast<const AVCodecID>(0), nullptr                       }
};

static struct {
  const AVSampleFormat avsf;
  const char*          name;
} s_avSampleFormatNames[] = {
  ...
  { static_cast<const AVSampleFormat>(0), nullptr }
};
...
void printAvError(const char *msg, int err);

} /* namespace logging */
} /* namespace mxp */

#endif /* LOGGING_LOGAVCODEC_H */
