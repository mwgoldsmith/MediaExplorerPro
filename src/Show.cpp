...
Show::Show( MediaExplorerPtr ml, sqlite::Row& row )
    : m_ml( ml )
{
    row >> m_id
        >> m_name
...
Show::Show( MediaExplorerPtr ml, const std::string& name )
    : m_ml( ml )
    , m_id( 0 )
    , m_name( name )
    , m_releaseDate( 0 )
...
std::shared_ptr<Show> Show::create( MediaExplorerPtr ml, const std::string& name )
{
