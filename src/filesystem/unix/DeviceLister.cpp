...
std::string DeviceLister::DeviceFromDeviceMapper(const std::string& devicePath) const {
  ...
  if (linkSize < 0) {
    std::stringstream err;
    err << "Failed to resolve device -> mapper link: " << devicePath << " (" << strerror(errno) << ')';
    throw std::runtime_error(err.str());
  }
  linkPath[linkSize] = 0;
  ...
  try {
    ...
    for (const auto& p : mountpoints) {
      ...
      if (it != end(devices)) {
        ...
        LOG_INFO("Failed to find device for mountpoint ", mountpoint, ". Attempting to resolve using device mapper");
